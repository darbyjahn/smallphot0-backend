...
const DATA_PATH = "/var/data"; // this is your persistent disk mount
...
if (!fs.existsSync(GALLERIES_PATH)) fs.mkdirSync(GALLERIES_PATH, { recursive: true });
...
// Serve galleries from persistent disk
app.use("/galleries", express.static(GALLERIES_PATH));

/* ---------- HELPERS ---------- */
const loadGalleries = () => JSON.parse(fs.readFileSync(GALLERIES_FILE, "utf8"));
const saveGalleries = (d) => fs.writeFileSync(GALLERIES_FILE, JSON.stringify(d, null, 2));

function ensureGalleryExists(username, title = username, bg = "#ffffff", text = "#000000") {
  ...
  if (!fs.existsSync(base)) {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ title, bg_color: bg, text_color: text, items: [] }, null, 2)
    );
...
// List galleries
app.get("/api/galleries", (req, res) => res.json(loadGalleries().users));
...
app.post("/api/upload/:user", async (req, res) => {
  try {
    ...
    for (let i = 0; i < uploadList.length; i++) {
      ...
      gallery.items.push({ stored: safeName, batch, seq: i, type: isVideo ? "video" : "image" });
      ...
      if (isVideo) {
        ...
        await new Promise((ok, fail) => {
          ...
          exec(cmd, (err) => {
            ...
            const last = gallery.items[gallery.items.length - 1];
            last.stored = mp4Name;
            last.type = "video";
            ok();
          });
